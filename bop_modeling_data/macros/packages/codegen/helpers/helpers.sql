...
{% macro codegen__get_model_dependencies(model_name) %}
...
{% macro codegen__add_model_column_descriptions_to_dict(resource_type, model_name, dict_with_descriptions={}) %}
...
{% macro codegen__build_dict_column_descriptions(model_name) %}
    {% if execute %}
        {% set glob_dict = {} %}
        {% for full_model in codegen__get_model_dependencies(model_name) %}
            {% do codegen__add_model_column_descriptions_to_dict(
...
{% macro codegen__get_models(directory=None, prefix=None) %}
    {% set model_names=[] %}
...
{% macro codegen__data_type_format_source(column) -%}
  {{ return(adapter.dispatch('codegen__data_type_format_source')(column)) }}
{%- endmacro %}

{# format a column data type for a source #}
{% macro default__codegen__data_type_format_source(column) %}
    {% set formatted = codegen__format_column(column) %}
    {{ return(formatted['data_type'] | lower) }}
{% endmacro %}

{% macro codegen__data_type_format_model(column) -%}
  {{ return(adapter.dispatch('codegen__data_type_format_model')(column)) }}
{%- endmacro %}

{# format a column data type for a model #}
{% macro default__codegen__data_type_format_model(column) %}
    {% set formatted = codegen__format_column(column) %}
    {{ return(formatted['data_type'] | lower) }}
{% endmacro %}
