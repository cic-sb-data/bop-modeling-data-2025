...
def model(dbt, session):
    df_pchainid = pl.from_pandas(dbt.ref("policy_chain_ids")).lazy()
    policy_chains = df_pchainid.select('policy_chain_id').unique().sort('policy_chain_id')
    policy_symbols = df_pchainid.select('policy_sym').unique().sort('policy_sym')
